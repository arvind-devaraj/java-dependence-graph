...
/**
 * Class to compute the Dominance (or Post Dominance) Frontier
 * using Cytron's algorithm. Generalisation of basic Soot's dominance
 * frontier class.
 *
 ...
 * Graph</a>
 **/
public class TwoWayCytronDF implements DominanceFrontier
{
	private boolean dirn;
    protected DominatorTree dt;
    protected Map nodeToFrontier;

    public TwoWayCytronDF (DominatorTree dt, boolean dirn)
    {
		this.dirn = dirn;
        this.dt = dt;
	 ...
	 * Succ is successor for dominance and predecessor for post
	 * dominance.
     *
    ...
    protected void processNode(DominatorNode node)
    {
        ...
        // local
        {
            Iterator succsIt = dirn ? 
				dt.getSuccsOf(node).iterator() :
				dt.getPredsOf(node).iterator();
